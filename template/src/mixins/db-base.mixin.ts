...
export class DbBaseMixin {
    ...
    return {
      ...
      methods: {
        ...
        async entityChanged(type: string, json: any, ctx: Context) {
          const events = Object.keys({ ...this.schema.events }).filter((event) => event.startsWith('cache.clean.'));
  ...
  private getLocalAdapter(schema: ServiceSchema, useFile = false) {
    ...
    const file = useFile ? { filename: `./data/${this.dbInfo.dbname}_${this.collection}.db` } : undefined;
    return {
      ...schema,
  ...
  private getMongoAdapter(schema: ServiceSchema) {
    return {
      ...schema,
      adapter: new AdapterMongo(this.getDBUri(), { useUnifiedTopology: true, authSource: 'admin', dbName: this.dbInfo.dbname }),
