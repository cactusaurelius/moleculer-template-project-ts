...
import { ProductsRecordParams } from '../../../src/types';
...
describe('Test "products" service', () => {
  describe('Test actions', () => {
    ...
    const record: ProductsRecordParams = {
      name: 'Awesome item',
      price: 999
    };
    let newID: IProduct['_id'];
    ...
    it('should add the new item', async () => {
      const res = await broker.call<IProduct, ProductsRecordParams>('products.create', record);
      expect(res).toEqual({
    ...
    it('should remove the updated item', async () => {
      const res = await broker.call<number, { id: IProduct['_id'] }>('products.remove', { id: newID });
      expect(res).toBe(1);

      const res2 = await broker.call<number>('products.count');
      expect(res2).toBe(0);
